# -*- coding: utf-8 -*-
"""
ItWorksOnYourMachineToo Logging Utilities - Utilitaires de journalisation
=======================================================

Ce module fournit des utilitaires de journalisation pour ItWorksOnYourMachineToo :
...
class ItWorksOnYourMachineTooLogger:
    """Gestionnaire de journalisation pour ItWorksOnYourMachineToo."""

    def __init__(self, name: str = "ItWorksOnYourMachineToo", log_level: str = "INFO"):
        self.logger = logging.getLogger(name)
        self.logger.setLevel(getattr(logging, log_level.upper()))
        self.log_dir = Path.home() / ".itworks" / "logs"
        self.log_dir.mkdir(parents=True, exist_ok=True)
        self.formatter = logging.Formatter("%(asctime)s - %(name)s - %(levelname)s - %(message)s")
        self.file_handler = logging.FileHandler(self.log_dir / f"iwym_{datetime.now().strftime('%Y%m%d')}.log")
        self.file_handler.setFormatter(self.formatter)
        self.logger.addHandler(self.file_handler)
        self.console_handler = None
        if os.getenv("IWYM_DEBUG", "").lower() in ("1", "true", "yes"):
...
class PerformanceMetrics:
    """Gestionnaire de métriques de performance pour ItWorksOnYourMachineToo."""
    ...
    def save_metrics(self):
        ...
        metrics_file = Path.home() / ".itworks" / "metrics.json"
        try:
...
_itworks_logger = None
_performance_metrics = PerformanceMetrics()


def get_logger(name: str = "ItWorksOnYourMachineToo", log_level: str = "INFO") -> logging.Logger:
    """Retourne une instance de logger pour ItWorksOnYourMachineToo."""
    global _itworks_logger
    if _itworks_logger is None:
        _itworks_logger = ItWorksOnYourMachineTooLogger(name, log_level)
    return _itworks_logger.get_logger()
...
def log_operation(operation: str, details: Dict[str, Any]):
    ...
    global _itworks_logger
    if _itworks_logger is None:
        _itworks_logger = ItWorksOnYourMachineTooLogger()
    _itworks_logger.log_operation(operation, details)
...
def log_error(operation: str, error: Exception, details: Dict[str, Any] = None):
    ...
    if _itworks_logger is None:
        _itworks_logger = ItWorksOnYourMachineTooLogger()
    _itworks_logger.log_error(operation, error, details)
